...
function ImageCarousel({ images, imageClassName = "h-[25rem]" }) {
  ...
  return (
    <div className="relative">
      <img src={images[currentIndex]} alt="Housing" className={`w-full object-cover rounded-lg ${imageClassName}`} />
      {images.length > 1 && (
        <>
          <button onClick={handlePrev} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white text-gray-700 px-2 py-1 rounded-full shadow">
            ◀
          </button>
          <button onClick={handleNext} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white text-gray-700 px-2 py-1 rounded-full shadow">
            ▶
          </button>
        </>
      )}
...
export default function Housing() {
  ...
  const applyFilters = () => {
    const res = listings.filter((l) => meetsBeds(l.bedsCount, filterBeds) && meetsBaths(l.bathsCount, filterBaths) && l.myPrice <= filterPrice);
    ...
    <section id="housing" className="min-h-screen bg-white">
      <nav className="flex justify-between items-center bg-white p-4 shadow-md rounded-lg">
        <div className="flex space-x-6">
          <Link href="/">
            <Image src="/images/Huddle_Social_White_Background.png" alt="Huddle Social Logo" width={40} height={40} className="rounded-full object-cover" />
          </Link>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredListings.map((l) => {
                ...
                return (
                  <div key={l.id} onClick={() => setSelectedListing(l)} className="relative bg-white shadow-md rounded-lg overflow-hidden cursor-pointer hover:shadow-lg transition">
                    {l.sublettingTrue && (
                      <span className="absolute top-2 left-2 bg-green-500 text-white px-3 py-1 rounded-full text-sm">Sublet</span>
                    )}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <p className="text-green-600 text-sm">
                          {l.moveInDate ? l.moveInDate.toDate().toLocaleDateString("en-US", { month: "long", year: "numeric" }) : "No move-in date"}
                        </p>
                        <span
                          className={`text-sm font-semibold px-3 py-1 rounded-full text-white ${
                            l.genderType === "Male" ? "bg-blue-500" : l.genderType === "Female" ? "bg-pink-500" : "bg-purple-500"
                          }`}
                        >
                          {l.genderType || "Co-ed"}
                        </span>
      ...
      {isLoggedIn && selectedListing && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setSelectedListing(null)}>
          <div className="relative bg-white w-11/12 max-w-3xl rounded-md shadow-lg p-8 pt-10 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setSelectedListing(null)} className="absolute top-3 right-3 text-black text-3xl font-bold hover:scale-110 transition">
              &times;
            </button>
            {(() => {
              ...
              return urls.length ? (
                ...
                <div className="w-full h-[20rem] bg-gray-200 rounded-lg flex items-center justify-center text-gray-500">No Image Available</div>
              );
            })()}
            <h2 className="text-3xl font-semibold mt-6 text-black">{selectedListing.myHousingName}</h2>
            ...
            <p className="mt-2 text-black">{selectedListing.descriptionText || "No description provided"}</p>
            <hr className="my-4 border-gray-300" />
            ...
            <p className="text-black mt-2">
              {selectedListing.includesUtilities || selectedListing.includesAmenities
                ? `${selectedListing.includesUtilities ? "Utilities Included" : ""}${selectedListing.includesUtilities && selectedListing.includesAmenities ? " | " : ""}${
            ...
            <div className="mt-2 text-black">
              <p>
                ...
                {selectedListing.moveInDate.toDate().toLocaleDateString("en-US", { month: "long", year: "numeric" })}
              </p>
              <p className="mt-2">
                <strong>Move-out Date:</strong>{" "}
                {selectedListing.moveOutDate.toDate().toLocaleDateString("en-US", { month: "long", year: "numeric" })}
              </p>
            </div>
            <button onClick={() => alert(`Email: ${posterEmail}`)} className="w-full bg-blue-500 text-white py-2 rounded-lg mt-6">
              Get Email
            </button>
          </div>
        </div>
      ...
      {isLoggedIn && showFilterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setShowFilterModal(false)}>
          <div className="absolute inset-0 bg-black bg-opacity-30" />
          <div className="relative w-11/12 max-w-md bg-white rounded-md shadow-lg p-8 pt-10 z-50" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowFilterModal(false)} className="absolute top-3 right-3 text-black text-3xl font-bold hover:scale-110 transition">
              &times;
            </button>
            ...
            <input type="range" min="0" max="5000" step="100" value={filterPrice} onChange={(e) => setFilterPrice(Number(e.target.value))} className="w-full accent-purple-500 mb-6" />
            <label className="block text-sm text-black mb-1">
              Beds: <strong>{filterBeds}</strong>
            </label>
            <select className="w-full mb-4 border border-gray-300 rounded-md p-2 focus:outline-none text-black" value={filterBeds} onChange={(e) => setFilterBeds(e.target.value)}>
            ...
            <select className="w-full mb-6 border border-gray-300 rounded-md p-2 focus:outline-none text-black" value={filterBaths} onChange={(e) => setFilterBaths(e.target.value)}>
              <option value="any">Any</option>
              <option value="1">1</option>
            ...
            <div className="flex justify-end space-x-2">
              <button onClick={clearFilters} className="px-3 py-1 rounded-full bg-white border border-black text-black font-bold hover:bg-gray-100 transition">
                Clear
              </button>
              <button onClick={() => setShowFilterModal(false)} className="px-3 py-1 rounded-full bg-white border border-black text-black font-bold hover:bg-gray-100 transition">
                Cancel
              </button>
              <button onClick={applyFilters} className="px-3 py-1 rounded-full bg-gray-300 text-black font-bold hover:scale-105 transition">
                Apply
              </button>
            </div>
          </div>
